  ...
  useEffect(() => {
    ...
    const loadDashboardData = async () => {
      ...
      try {
        ...
        const         const rawDigestIds = profile?.digest_opportunity_ids         let digestIds: string[] = []         if (Array.isArray(rawDigestIds)) {           digestIds = rawDigestIds.map((id: any) => String(id)).filter(Boolean)         } else if (typeof rawDigestIds === 'string' && rawDigestIds.trim()) {           try {             const parsed = JSON.parse(rawDigestIds)             if (Array.isArray(parsed)) {               digestIds = parsed.map((id: any) => String(id)).filter(Boolean)             }           } catch {             digestIds = []           }         }          if (digestIds.length === 0) {           setModalOpportunities([])           setRecentRelevantOpps([])           setHasNoRecentRelevant(true)         } else {           const { data: opps } = await supabase             .from('opportunities')             .select('*')             .in('id', digestIds)             .eq('is_active', true)            if (opps && isMounted) {             const formattedOpps = opps.map((opp: Opportunity) => {               const mappedOpp: MatchOpportunity = {                 id: opp.id,                 title: opp.title,                 type: opp.type || 'open_call',                 eligible_countries: opp.eligible_countries || ['Україна', 'Worldwide'],                 deadline: opp.deadline || new Date().toISOString(),                 fee: Number(opp.fee_amount || opp.cost_amount || opp.org_fee || 0),                 currency: 'USD',                 techniques: Array.isArray(opp.techniques) ? opp.techniques : [],                 themes: Array.isArray(opp.themes) ? opp.themes : [],                 required_level: opp.required_level || undefined,               }               const match = calculateMatch(artistProfileData, mappedOpp)               return {                 id: opp.id,                 title: opp.title,                 description: opp.raw_description || opp.description || '',                 created_at: opp.created_at || new Date().toISOString(),                 deadline: opp.deadline || undefined,                 link_url: opp.source_url || opp.link || opp.link_url || opp.url || undefined,                 matchScore: match.score,                 matchReasons: match.reasons,                 recommendedAction: match.recommendedAction,               }             })              const order = new Map(digestIds.map((id, index) => [id, index]))             formattedOpps.sort((a, b) => {               const ai = order.has(a.id) ? (order.get(a.id) as number) : 999               const bi = order.has(b.id) ? (order.get(b.id) as number) : 999               if (ai !== bi) return ai - bi               return (b.matchScore || 0) - (a.matchScore || 0)             })              setModalOpportunities(formattedOpps)             setHasNoRecentRelevant(formattedOpps.length === 0)             setRecentRelevantOpps(formattedOpps)           } else if (isMounted) {             setModalOpportunities([])             setRecentRelevantOpps([])             setHasNoRecentRelevant(true)           }         } = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString()
